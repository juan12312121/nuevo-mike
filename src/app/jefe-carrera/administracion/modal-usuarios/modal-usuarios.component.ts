...
export class ModalUsuariosComponent implements OnInit, OnChanges {
  ...
  private checkJefeCarrera() {
    ...
    if (currentUser && currentUser.rol_id === 4) {
      ...
      // Si estamos creando un usuario nuevo, auto-asignamos la carrera
      if (!this.usuarioId) {
        this.usuario.carrera_id = this.jefeCarreraId;
      }
    }
  }
...
private obtenerGrupos(): void {
  ...
  onSubmit(form: NgForm) {
    ...
    // Construye el payload base combinando el objeto usuario con los valores del formulario
    const payload: any = {
      ...this.usuario,
      ...form.value,
