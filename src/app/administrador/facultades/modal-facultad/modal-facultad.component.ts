  ...
  cargarEscuelas(): void {
    this.escuelasService.verEscuelas().subscribe({
      next: (data) => {
        ...
        // Auto-seleccionar si solo hay una escuela disponible y es registro nuevo
        if (this.escuelas.length === 1 && this.nuevaFacultad.escuela_id === 0) {
          this.nuevaFacultad.escuela_id = this.escuelas[0].id;
        }
      },
